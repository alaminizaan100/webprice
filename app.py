...
@app.route('/')
def index():
    min_arbitrage = 0.5  # minimum arbitrage percentage to consider

    # Load markets
    binance_markets = requests.get(
        f'{BINANCE_BASE_URL}/api/v3/exchangeInfo').json()
    poloniex_markets = requests.get(
        f'{POLONIEX_BASE_URL}?command=returnTicker').json()
    ...
    for symbol in binance_markets['symbols']:
        if symbol['status'] == 'TRADING':
            ...
            asset_info = requests.get(
                f'{BINANCE_BASE_URL}/sapi/v1/asset/assetDetail?asset={asset}',
                headers={
                    'X-MBX-APIKEY': BINANCE_API_KEY}).json()
            if asset_info.get(
                asset,
                {}).get(
                'depositStatus',
                False) and asset_info.get(
                asset,
                {}).get(
                'withdrawStatus',
                    False):
                ticker = requests.get(
                    f'{BINANCE_BASE_URL}/api/v3/ticker/24hr?symbol={symbol["symbol"]}').json()
    ...
    fees = {}
    withdrawal_fees = requests.get(
        f'{BINANCE_BASE_URL}/sapi/v1/asset/assetDetail',
        headers={
            'X-MBX-APIKEY': BINANCE_API_KEY}).json()
    for symbol in filtered_coins:
        asset = symbol.split('/')[0]
        fees[symbol] = {
            'withdrawal_fee': withdrawal_fees.get(
                asset,
                {}).get(
                'withdrawFee',
                0),
    ...
    for symbol in filtered_coins:
        binance_price = float(requests.get(
            f'{BINANCE_BASE_URL}/api/v3/ticker/price?symbol={symbol}').json()['price'])
        ...
        if poloniex_symbol in poloniex_markets:
            ...
            binance_min_notional = float([filter for filter in binance_markets['symbols'] if filter['symbol'] == symbol][0]['filters'][3]['minNotional'])
            poloniex_min_trade_size = float(poloniex_markets[poloniex_symbol]['lowestAsk'])
            ...
            # Arbitrage up to 10 times minimum trade size
            max_trade_size = min(
                filtered_coins[symbol]['volume'],
                10 * min_trade_size)

            # Calculate potential arbitrage profit
            arbitrage = (poloniex_price - binance_price) / binance_price * 100
            # Deduct trading fees, transfer fees, and minimum trade size
            arbitrage -= fees[symbol]['binance_trading_fee'] + \
                fees[symbol]['poloniex_trading_fee'] + fees[symbol]['withdrawal_fee']
